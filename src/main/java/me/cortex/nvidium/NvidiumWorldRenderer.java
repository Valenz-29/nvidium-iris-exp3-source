...
public class NvidiumWorldRenderer {
    ...
    public NvidiumWorldRenderer() {
        ...
        Nvidium.LOGGER.info(
                "Using {} MiB persistent RAM-backed geometry upload staging",
                this.uploadStream.getCapacityMiB()
        );

        this.downloadStream = new DownloadTaskStream(device, frames, 8_000_000);
        ...
        this.sectionManager = new SectionManager(
                ...
                uploadStream,
                Nvidium.config.use_sodium_vertex_format
    ...
    public void addDebugInfo(ArrayList<String> debugInfo) {
        ...
        debugInfo.add(
                "Upload staging: " + this.uploadStream.getCapacityMiB() + " MiB, forced waits: "
                        + this.uploadStream.getForcedWaitCount()
        );
        debugInfo.add(
                "Regions: " + sectionManager.getRegionManager().regionCount()
    ...
    private void update_allowed_memory() {
        if (Nvidium.config.automatic_memory) {
            max_geometry_memory = (glGetInteger(GL_GPU_MEMORY_INFO_CURRENT_AVAILABLE_VIDMEM_NVX) / 1024)
                    + (sectionManager == null
                    ? 0
                    : sectionManager.terrainAreana.getMemoryUsed() / (1024 * 1024));
